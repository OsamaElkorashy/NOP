...
namespace Nop.Web.Factories
{
    public partial class CatalogModelFactory : ICatalogModelFactory
    {
        ...
        public virtual SearchModel PrepareSearchModel(SearchModel model, CatalogPagingFilteringModel command)
        {
            ...
            var discounts = _discountService.GetAllDiscounts(Core.Domain.Discounts.DiscountType.AssignedToSkus);
            if (discounts.Any())
            {
                ...
                foreach (var m in discounts)
                    model.AvailableDiscounts.Add(new SelectListItem
                    {
                        Value = m.Id.ToString(),
                        Text = _localizationService.GetLocalized(m, x => x.Name),
                        Selected = model.did == m.Id
                    });
            }
        ...
        /// </summary>
        /// <returns>Search box model</returns>
        public virtual DisountProductsModel PrepareDiscountProductsModel(int discountId, CatalogPagingFilteringModel command)
        {
            var model = new DisountProductsModel();
            //sorting
            PrepareSortingOptions(model.PagingFilteringContext, command);
            //view mode
            PrepareViewModes(model.PagingFilteringContext, command);
        ...
        ///// </summary>
        ///// <returns>Search box model</returns>
        //public virtual DisountProductsModel PrepareDiscountProductsModel(string discountName)
        //{
        //    var discount = _discountService.GetDiscountByName(discountName);
        //    var model = new DisountProductsModel();
        //    var products = _productService.GetProductsWithAppliedDiscount(discount.Id);
        //    model.Products = _productModelFactory.PrepareProductOverviewModels(products).ToList();
        //    model.did = discount.Id;
        //    return model;
        //}
    }
}